...
class sfBasicSecurityFilter extends sfFilter
{
  ...
  {
    //start - thongnq1 - 07/05/2013 - fix bug khi chuyen ve trang login van submit du lieu cu ->loi trang validator
    //check request method is POST
    $method = sfContext::getInstance()->getRequest()->getMethod();
    if($method == "POST"){
      $this->context->getController()->redirect(sfConfig::get('sf_login_module').'/'. sfConfig::get('sf_login_action'));
    }
